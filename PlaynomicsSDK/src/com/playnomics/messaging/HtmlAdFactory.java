...
public class HtmlAdFactory {
	public HtmlAd createDataFromBytes(byte[] data) throws UnsupportedEncodingException, JSONException{
		...
		String impressionUrl = cleanJSONString(firstAdData, "impressionUrl", false);
		...
		String contentBaseUrl = cleanJSONString(firstAdData, "contentBaseUrl", false);
		
		return new HtmlAd(position, closeButton, target, clickLink, clickUrl, impressionUrl, closeUrl, contentBaseUrl, htmlContent);
	}
	
	private Position getPositionFromResponse(JSONObject response) throws JSONException{
		JSONObject positionData = response.getJSONObject("position");
		String positionTypeString = cleanJSONString(positionData, "positionType", false);
	...
	private Target getTargetFromResponse(JSONObject response) throws JSONException {
		JSONObject targetJSONData = response.getJSONObject("target");
		String targetTypeString = cleanJSONString(targetJSONData, "targetType", false);
		TargetType targetType = toTargetType(targetTypeString);
		
		if(targetType == TargetType.URL){
			String targetUrl = cleanJSONString(targetJSONData, "targetUrl", true);
	...
	private CloseButton getCloseButtonFromResponse(JSONObject response) throws JSONException{
		...
		if(closeButtonType.equals("native")){
			String closeImageUrl = closeButtonJSONData.getString("closeImageUrl");
	...
	private TargetType toTargetType(String targetTypeString) throws JSONException{
	...
	private String cleanJSONString(JSONObject json, String key, boolean optional) throws JSONException{
