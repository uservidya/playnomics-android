...
public class ContextWrapperTest {
	...
	public void setUp() throws Exception {
		...
		when(contextMock.getSharedPreferences(eq(ContextWrapper.CACHE_NAME), anyInt())).thenReturn(preferencesMock);
	...
	public void testGetPreviousSessionId(){
		when(preferencesMock.getLong(ContextWrapper.SESSION_ID_KEY, ContextWrapper.DEFAULT_CACHE_VALUE)).thenReturn(10L);
	...
	public void testGetPreviousSessionIdNull(){
		when(preferencesMock.getLong(ContextWrapper.SESSION_ID_KEY, ContextWrapper.DEFAULT_CACHE_VALUE)).thenReturn((long)ContextWrapper.DEFAULT_CACHE_VALUE);
		LargeGeneratedId previousSessionId =  contextWrapper.getPreviousSessionId();
	...
	public void testSetPreviousSessionId(){
		...
		verify(editorMock).putLong(ContextWrapper.SESSION_ID_KEY, sessionId.getId());
		verify(editorMock).commit();
	}

	@Test
	public void testGetLastEventTime(){
		when(preferencesMock.getLong(ContextWrapper.LAST_EVENT_TIME_CACHE_KEY, ContextWrapper.DEFAULT_CACHE_VALUE)).thenReturn(0L);
		EventTime lastEventTime =  contextWrapper.getLastEventTime();
		assertEquals("Last Event Time is restored", 0L, lastEventTime.getTimeInMillis());
	}
	
	@Test
	public void testGetLastEventTimeNull(){
		when(preferencesMock.getLong(ContextWrapper.LAST_EVENT_TIME_CACHE_KEY, ContextWrapper.DEFAULT_CACHE_VALUE)).thenReturn((long)ContextWrapper.DEFAULT_CACHE_VALUE);
		EventTime lastEventTime =  contextWrapper.getLastEventTime();
	...
	public void testSetLastEventTime(){
		...
		verify(editorMock).putLong(ContextWrapper.LAST_EVENT_TIME_CACHE_KEY, lastEventTime.getTimeInMillis());
		verify(editorMock).commit();
	}
	
	@Test
	public void testGetLastSessionStartTimeTime(){
		when(preferencesMock.getLong(ContextWrapper.SESSION_START_TIME_CACHE_KEY, ContextWrapper.DEFAULT_CACHE_VALUE)).thenReturn(0L);
	...
	public void testGetLastSessionStartTimeNull(){
		when(preferencesMock.getLong(ContextWrapper.SESSION_START_TIME_CACHE_KEY, ContextWrapper.DEFAULT_CACHE_VALUE)).thenReturn((long)ContextWrapper.DEFAULT_CACHE_VALUE);
		EventTime startTime =  contextWrapper.getLastSessionStartTime();
	...
	public void testSetLastSessionStartTime(){
		...
		verify(editorMock).putLong(ContextWrapper.SESSION_START_TIME_CACHE_KEY, startTime.getTimeInMillis());
		verify(editorMock).commit();
	}
	...
	public void testGetPushId(){
		...
		when(preferencesMock.getString(ContextWrapper.PUSH_ID_CACHE_KEY, null)).thenReturn(pushId);
	...
	public void testSynchronizeNoChanges(){
		...
		when(utilMock.getApplicationVersionFromContext(contextMock)).thenReturn(currentVersion);
		when(preferencesMock.getInt(ContextWrapper.APP_VERSION_CACHE_KEY, ContextWrapper.DEFAULT_CACHE_VALUE)).thenReturn(oldVersion);
		
		assertFalse("No changes to device settings", contextWrapper.synchronizeDeviceSettings());
	}
	...
	public void testSynchronizeChanges(){
		...
		when(preferencesMock.getInt(ContextWrapper.APP_VERSION_CACHE_KEY, ContextWrapper.DEFAULT_CACHE_VALUE)).thenReturn(oldVersion);
		//push id is reset
		assertTrue("Changes to device settings", contextWrapper.synchronizeDeviceSettings());
		//the push ID is cleared
		verify(editorMock).putString(ContextWrapper.PUSH_ID_CACHE_KEY, null);
		//app version is set
		verify(editorMock).putInt(ContextWrapper.APP_VERSION_CACHE_KEY, currentVersion);
