...
public class AssetClientTest {
	...
	public void testSuccess() throws IOException {
		when(httpFactoryMock.startConnectionForUrl("http://google.com"))
				.thenReturn(connectionMock);

		URL url = this.getClass().getResource(
				"/sample-internal-ad-all-html-target-data.json");
		File sampleJsonFile = new File(url.getFile());
		FileInputStream stream = new FileInputStream(sampleJsonFile);
		when(connectionMock.getResponseCode()).thenReturn(
				HttpURLConnection.HTTP_OK);
		when(connectionMock.getInputStream()).thenReturn(stream);

		AssetResponse response = client.requestAsset("http://google.com", null,
				null);
		assertEquals("Request is successful", ResponseStatus.SUCCESS,
				response.getStatus());
		...
		assertArrayEquals("Data is loaded as expected", expected,
				response.getData());
	}
	...
	public void testFailureConnectionFailed() throws IOException {
		...
		AssetResponse response = client.requestAsset("http://google.com", null,
				null);
		assertEquals("Request fails", ResponseStatus.FAILURE,
				response.getStatus());
	}
